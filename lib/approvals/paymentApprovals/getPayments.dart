    ...
    for (int i = 0; i < empresa.length; i++) {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        for (int i = 0; i < nQtdTit; i++) {
          ...
          rateio = [];
          for (int rat = 0;
              rat < response.data["titulos"][i]["rateio"].length;
              rat++) {
            if (response.data["titulos"][i]["rateio"][rat].length > 0) {
              rateio.add([
                response.data["titulos"][i]["rateio"][rat]["cc"],
                response.data["titulos"][i]["rateio"][rat]["percentual"],
                "R\$ " +
                    formatMoney(
                        "${response.data["titulos"][i]["rateio"][rat]["valor"]}")
              ]);
            }
          }
          if (cgc.length == 0) {
          ...
          if (titpai == '') {
            ...
            dadosAdicionais.add([
              ...
              numero,
              parcela,
              " R\$ " + formatMoney("${response.data["titulos"][i]["saldo"]}"),
              vencimento,
              codfil,
              nomefil,
              cgc + "          " + ie,
